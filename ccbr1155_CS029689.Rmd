...
peak_anno_4 = main_prep_qc(contrast_id,exclusion_list)
```
...
peak_anno_9 = main_prep_qc(contrast_id,exclusion_list)
```

## **Data Processing**
Reviewing counts of significant peaks guides analysis parameters downstream.

```{r collapsedf, echo=FALSE, warning=FALSE,warning=FALSE, message=FALSE}
#read gene list
...
```{r graphics1,echo=FALSE, warning=FALSE,warning=FALSE, message=FALSE}
chipseeker_plots(peak_anno_4)
```

### 5-3_H3K9me3_IFNb_vs_HN6_H3K9me3_IFNb

```{r graphics2,echo=FALSE, warning=FALSE,warning=FALSE, message=FALSE}
chipseeker_plots(peak_anno_9)
```
